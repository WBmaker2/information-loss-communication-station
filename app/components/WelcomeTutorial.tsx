...
export function Tutorial({ onDone, onBack }: { onDone: () => void; onBack: () => void }) {
  ...
  return <section className="card lesson"><p className="eyebrow">연습 활동 · {step + 1}/2</p><h1>연습 활동</h1><p>{TUTORIAL_CASE.purpose}</p><p className="muted">빠진 말은 이전 문장에서, 다른 변화는 다음 문장에서 골라요.</p><ol className="activity-steps" aria-label="연습 활동 순서"><li><b>1.</b> 달라진 말 고르기</li><li><b>2.</b> 어떻게 달라졌나요?</li><li><b>3.</b> 왜 그렇게 생각했나요?</li></ol><fieldset className="expression-choice"><legend>1. 달라진 말 고르기</legend><p className="muted">이전과 다음 문장에서 관련 말을 눌러 보세요.</p><div className="comparison"><MessageCard title="이전 문장" stage={stages[step]} selectable selected={segments} onSelect={(id) => toggle(id, segments, setSegments)} /><MessageCard title="다음 문장" stage={stages[step + 1]} selectable selected={segments} onSelect={(id) => toggle(id, segments, setSegments)} /></div></fieldset><fieldset><legend>2. 어떻게 달라졌나요?</legend><div className="choice-row">{(["omission", "unsupported-addition", "meaning-shift", "meaning-preserving"] as ChangeType[]).map((type) => <label key={type}><input type="radio" name="tutorial-change-type" checked={changeType === type} onChange={() => setChangeType(type)} />{changeNames[type]}</label>)}</div></fieldset><fieldset><legend>3. 왜 그렇게 생각했나요?</legend><div className="check-grid">{TUTORIAL_CASE.meaningUnits.map((unit) => <label key={unit.id}><input type="checkbox" checked={evidence.includes(unit.id)} onChange={() => toggle(unit.id, evidence, setEvidence)} />{labels[unit.kind]} · {unit.canonicalMeaning}</label>)}</div></fieldset><SelectionSummary selectedSegmentCount={segments.length} selectedEvidenceCount={evidence.length} onClear={clearSelections} guidanceId={nextGuidanceId} showNextGuidance={!canAdvance} /><div className="button-row"><button className="primary" onClick={check}>내 답 확인</button><button disabled={!canAdvance} aria-describedby={!canAdvance ? nextGuidanceId : undefined} onClick={next}>{step === 0 ? "다음 비교" : "사건 임무로"}</button><button onClick={restart}>처음부터 다시</button></div><p ref={feedbackRef} tabIndex={-1} className="feedback" aria-live="polite">{feedback}</p><div className="back-row"><button className="text-button" onClick={onBack}>시작 화면으로</button><button className="text-button" onClick={onDone}>연습 활동 건너뛰기</button></div></section>;
}
