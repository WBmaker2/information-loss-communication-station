...
export function Tutorial({ onDone }: { onDone: () => void }) {
  ...
  return <section className="card lesson"><p className="eyebrow">안내 활동 · {step + 1}/2</p><h1>{TUTORIAL_CASE.title}</h1><p>{TUTORIAL_CASE.purpose}</p><p className="muted">빠짐은 이전 문장에서, 다른 변화는 다음 문장에서 조각을 고릅니다.</p><div className="comparison"><MessageCard title="이전 문장" stage={stages[step]} selectable selected={segments} onSelect={(id) => toggle(id, segments, setSegments)} /><MessageCard title="다음 문장" stage={stages[step + 1]} selectable selected={segments} onSelect={(id) => toggle(id, segments, setSegments)} /></div><fieldset><legend>변화 종류</legend><div className="choice-row">{(["omission", "unsupported-addition", "meaning-shift", "meaning-preserving"] as ChangeType[]).map((type) => <label key={type}><input type="radio" name="tutorial-change-type" checked={changeType === type} onChange={() => setChangeType(type)} />{changeNames[type]}</label>)}</div></fieldset><fieldset><legend>근거 뜻</legend><div className="check-grid">{TUTORIAL_CASE.meaningUnits.map((unit) => <label key={unit.id}><input type="checkbox" checked={evidence.includes(unit.id)} onChange={() => toggle(unit.id, evidence, setEvidence)} />{labels[unit.kind]} · {unit.canonicalMeaning}</label>)}</div></fieldset><div className="button-row"><button className="primary" onClick={check}>판정 확인</button><button disabled={!canAdvance} onClick={next}>{step === 0 ? "다음 비교" : "사건 임무로"}</button><button onClick={restart}>처음부터 다시</button></div><p ref={feedbackRef} tabIndex={-1} className="feedback" aria-live="polite">{feedback}</p><button className="text-button" onClick={onDone}>안내 활동 건너뛰기</button></section>;
}
